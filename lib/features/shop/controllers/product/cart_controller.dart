...
class CartController extends GetxController {
  ...
  void addToCart(ProductModel product) {
    ...
    if (productQuantityInCart.value < 1) {
      ALoaders.customToast(message: "Selectionner la Quantité.");
      return;
    }
    ...
    if (product.productType == ProductType.variable.toString()) {
      if (variationController.selectedVariation.value.stock < 1) {
        ALoaders.warningSnackBar(
            title: "Oh Erreur!",
            message: "La variante sélectionnée est en rupture de stock.");
        return;
      }
    } else {
      if (product.stock < 1) {
    ...
    updateCart();
    ALoaders.customToast(message: "Votre produit a été ajouté au Panier.");
  }
  ...
  void removeFromCartDialog(int index) {
    Get.defaultDialog(
      ...
      middleText: "Etes Vous Sur de Vouloir Supprimé ce Produit?",
      onConfirm: () {
        /// Remove the item from the cart
        cartItems.removeAt(index);
        updateCart();
        ALoaders.customToast(message: "Produit Supprimé de Votre Panier.");
        Get.back();
      },
      onCancel: () => () => Get.back(),
    );
